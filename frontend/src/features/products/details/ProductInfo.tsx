...
const ProductInfo = ({ product }: Props) => {
	...
	return (
		<Segment.Group style={segmentStyle}>
			<Segment secondary>
				{product ? (
					<Header as={"h1"} style={{ color: "var(--onyx)" }}>
						{product.name}
					</Header>
				) : (
					<Placeholder fluid>
			...
			<Segment.Group horizontal size="big" style={segmentStyle}>
				{product ? (
					<>
						...
						<Segment compact>{Math.floor(Math.random() * (500 - 9 + 1) + 9)} avaliações</Segment>
						<Segment compact>{Math.floor(Math.random() * (2000 - 100 + 1) + 100)} vendidos</Segment>
					</>
				) : (
					<Placeholder />
				)}
